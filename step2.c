...
#define NWien 16 //MAX NUMBLE BLOCK step2 , Must be a power of 2
#define KK (K*K)
...
int main()
{
   ...
   {0.1924f, 0.2989f, 0.3846f, 0.4325f, 0.4325f, 0.3846f, 0.2989f, 0.1924f } };
	...
	sprintf(output_img_step2, "denoised_step2.png");
	...
	static float wien_tmp[NHard] = { 0.0f };
	...
	//! Loop on j_r
			...
			//for (unsigned ind_j = 0; ind_j < 1; ind_j++)
		{
			...
			for (unsigned n = 0; n < nSx_r; n++) {
				for (int p = 0; p < K; p++) {
					for (int q = 0; q < K; q++) {
						group_3D_table[n + ind_j * NWien][p][q] = group_3D_est[n + (p * K + q) * nSx_r];
					}
				}
			}
			//free(group_3D);
			...
			for (unsigned n = 0; n < nSx_r; n++)
			{
				for (unsigned p = 0; p < kHard; p++)
					for (unsigned q = 0; q < kHard; q++)
					{
						numerator[(index_h[k_r][n] + p)][(index_w[k_r][n] + q)] += kaiser_window[p][q]
							* weight_table[ind_j]
							* group_3D_table[n + ind_j * NWien][p][q];
						//printf(" ka %f ,W : %f, g: %f ", kaiser_window[p][q], weight_table[ind_j], group_3D_table[n + ind_j * NWien][p][q]);
	...
	if (write_png_f32(output_img_step2, img_out, (size_t)nx, (size_t)ny, (size_t)nc) != 0) {
		printf("... failed to save png image %s", output_img);
	}
